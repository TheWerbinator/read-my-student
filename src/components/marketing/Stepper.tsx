...
export function HeroStepper() {
  const steps: Step[] = useMemo(
    () => [
      {
        title: "Request",
        subtitle: "Student sends a request",
        icon: <FileText className='h-4 w-4' />,
      },
      {
        title: "Draft",
        subtitle: "Faculty permission granted",
        icon: <Edit3 className='h-4 w-4' />,
      },
      {
        title: "Approve",
        subtitle: "Faculty writes the letter",
        icon: <CheckCircle2 className='h-4 w-4' />,
      },
      {
        title: "Consent",
        subtitle: "Student reviews and approves",
        icon: <ShieldCheck className='h-4 w-4' />,
      },
      {
        title: "Share",
        subtitle: "Consent link, ready anywhere",
  ...
  const progress = steps.length <= 1 ? 0 : showIndex / (steps.length - 1);
  ...
  return (
    <div className='pointer-events-none select-none mx-auto max-w-xl md:block md:max-w-4xl rounded-3xl border border-white/20 bg-white/85 backdrop-blur shadow-xl shadow-black/10 px-5 py-3'>
      {/* One-line rail */}
      <div className='relative'>
        {/* Rail */}
        <div className='absolute left-5 right-5 top-1/4 -translate-y-1/2'>

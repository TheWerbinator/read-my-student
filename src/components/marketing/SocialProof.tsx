      ...
      <div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((s) => (
            ...
            >
              <div className="font-serif text-3xl md:text-4xl font-semibold text-[#0b1553]">
                {s.value}
              </div>
              <div className="mt-2 text-sm font-semibold text-[#5f6a93]">
                {s.label}
              </div>
            </div>
          ))}
...
function TestimonialCard({ t }: { t: Testimonial }) {
  ...
  return (
    <div className="rounded-2xl border border-black/10 bg-white px-7 py-8 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
      {/* big quote mark */}
      <div className="text-amber-200/90 font-serif text-6xl leading-none select-none">
        “
      </div>
      ...
      {/* quote */}
      <p className="mt-4 text-sm leading-relaxed text-[#3c466a] italic">
        {`"${t.quote}"`}
      </p>

      {/* footer */}
      <div className="mt-8 flex items-center gap-4">
        <div className="h-12 w-12 rounded-full bg-[#0b1553] text-white font-bold grid place-items-center">
          {t.initials}
        </div>

        <div className="leading-tight">
          <div className="font-semibold text-[#0a154a]">{t.name}</div>
          <div className="text-xs font-semibold text-[#5f6a93]">{t.title}</div>
          <div className="text-xs font-semibold text-amber-600">{t.org}</div>
        </div>
      </div>
    </div>
  );
